...
def get_classes(task):
    ...
    #jang-r
        ...
        return KnowledgeSelectionDataset, RobertaForMultipleChoice, run_batch_selection_train, run_batch_selection_eval

    elif task.lower() == "post-training":
...
def train(args, train_dataset, eval_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, run_batch_fn_train, run_batch_fn_eval) -> Tuple[int, float]:
    ...
    for _ in train_iterator:
        ...
        if args.local_rank in [-1, 0]:
            ...
            if args.task.lower() == "detection":
                current_score=results['precision']+results['recall']
            elif args.task.lower()=='selection':
                current_score=results['accuracy']
            else:
                current_score=0
                #current_score=-results['perplexity']
...
def evaluate(args, eval_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, run_batch_fn, desc="") -> Dict:
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating", disable=args.local_rank not in [-1, 0]):
        with torch.no_grad():
            ...
            if args.task in ["detection","selection"]:
    ...
    if args.task.lower() == "generation" or  args.task.lower() == "post-training"  :
        ...
        raise ValueError("args.task not in ['generation', 'selection', 'detection','post-training'], got %s" % args.task)
...
def main():
    ...
    parser.add_argument("--negative_sample_method", type=str, choices=["all", "mix","domain", "oracle"],
    ...
    #jang 
    #args.n_gpu = torch.cuda.device_count() if args.distributed else 1
    args.n_gpu=1
    args.device = device
