...
for log,label in zip(logs9,labels9):
    if label['target']==True:
        ...
        if (entity_id,doc_id) not in entdoc2uttr:
            entdoc2uttr[(entity_id,doc_id)]=[]
       
        try:
            input_text=log[-1]['text'].lower()
        except:
            input_text=log[0]['text'].lower()
        

        entity_name=kb10[domain][str(entity_id)]['name'].lower()

        if entity_name in input_text:
            input_text=input_text.replace(entity_name,"__%s__"%domain)
        else:
            for vari_enitity in kb9_entity_variation[domain]:
                if vari_enitity in input_text:
                    input_text=input_text.replace(vari_enitity,"__%s__"%domain)
                    break
        entdoc2uttr[(entity_id,doc_id)].append(input_text)
...
for domain in kb10:
    kb9_cnt=0
    kb10_cnt=0
    kb9_domain_q_dict[domain]=[]
    kb10_domain_q_dict[domain]=[]
    kb9_mapping_dict[domain]={}
        ...
        try:
            ...
            for doc_id in kb10[domain][entity_id]['docs']:
                ...
                if entity_name in title:
                    title=title.replace(entity_name,"__%s__"%domain)
                else:
                    for vari_enitity in kb9_entity_variation[domain]:
                        if vari_enitity in title:
                            title=title.replace(vari_enitity,"__%s__"%domain)
                            break
                title=' '.join(title.split())#공백 제거
                ...
                if kb10[domain][entity_id]['city']=="San Francisco" :
                    ...
                    kb10_cnt+=1
                    if title in kb10_domain_q_dict[domain]:
                        #이미 들어가 있으면 그 인덱스의 정보에다가 entitiy doc id 추가함.
                        kb10_mapping_dict[domain][kb10_domain_q_dict[domain].index(title)].append((entity_id,doc_id))
                    else:
                        #한번도 안들어 왓으면
                        #기존 길이의 인덱스로 값이 들어가고 거기에 맵핑
                        kb10_mapping_dict[domain][len(kb10_domain_q_dict[domain])]=[(entity_id,doc_id)]
                        kb10_domain_q_dict[domain].append(title)
                else:
                    kb9_cnt+=1

                    #나중에 빼줘야 하니까 그리고 다시 넣어줘야 하네.
                    #이경우는 대화 자체가 한번도 안나타난것.
                    if (int(entity_id),int(doc_id)) not in entdoc2uttr:
                        #print(entity_id,doc_id)
                        no_log_cnt+=1

                    if title in kb9_domain_q_dict[domain]:
                        #이미 들어가 있으면 그 인덱스의 정보에다가 entitiy doc id 추가함.
                        kb9_mapping_dict[domain][kb9_domain_q_dict[domain].index(title)].append((entity_id,doc_id))
                        #print()
                    else:
                        #한번도 안들어 왓으면
                        #기존 길이의 인덱스로 값이 들어가고 거기에 맵핑
                        kb9_mapping_dict[domain][len(kb9_domain_q_dict[domain])]=[(entity_id,doc_id)]
                        kb9_domain_q_dict[domain].append(title)
                        
                
        except:
            continue
            #print("None")
    print("kb9 unique query # of %s : %d and all query :%d"%(domain,len(kb9_domain_q_dict[domain]),kb9_cnt))
    print("kb9 no_log # of %s : %d "%(domain, no_log_cnt))
    print("kb10 unique query # of %s : %d and all query :%d"%(domain,len(kb10_domain_q_dict[domain]),kb10_cnt))
    print()
...
def mapping_kb9_kb10(mapping_dict,num_k):
    ...
    for domain in kb10:
        ...
        kb9_faqs=kb9_domain_q_dict[domain]
        ...
        for i,(topk_values,topk_indices) in enumerate(zip(topk_values_2d,topk_indices_2d)):
            ...
            for topk_val, topk_idx in zip(topk_values,topk_indices):#여기서 나중에 top k
                ...
                if topk_val > min_p:
                    kb10_key_list=kb10_mapping_dict[domain][int(topk_idx)]
                    for key in kb10_key_list:
                        if key not in mapping_dict:
                            mapping_dict[key]={"entdoc":[],"kb9":[],"kb10":[]}
                        mapping_dict[key]["entdoc"].extend(kb9_mapping_dict[domain][i])
                        
                        idx_tuple=(domain,i)
                        if idx_tuple not in mapping_dict[key]["kb9"]:
                            mapping_dict[key]["kb9"].append(idx_tuple)
            ...
            for topk_val, topk_idx in zip(topk_values,topk_indices):#여기서 나중에 top k
                ...
                if topk_val > min_p:
                    ...
                    for key in kb10_key_list:
                        ...
                        if kb9_mapping_dict[domain][int(topk_idx)][0] in mapping_dict[key]['entdoc']:
                            break
                        else:
                            mapping_dict[key]['entdoc'].extend(kb9_mapping_dict[domain][int(topk_idx)])
...
def mapping_kb10_kb10(mapping_dict,num_k):
            ...
            for topk_val, topk_idx in zip(topk_values,topk_indices):#여기서 나중에 top k
                ...
                if topk_val > min_p:
                    ...
                    for key in kb10_key_list:
                        ...
                        idx_tuple=(domain,int(topk_idx))
                        if idx_tuple not in mapping_dict[key]["kb10"]:
                            mapping_dict[key]["kb10"].append(idx_tuple)
...
def check_kb10_entity_link(mapping_dict):
    ...
    for (domain,entity_id,doc_id) in kb10_every_entity:
        if (entity_id,doc_id) in mapping_dict:
            average+=len(mapping_dict[(entity_id,doc_id)]["entdoc"])+len(mapping_dict[(entity_id,doc_id)]["kb9"])+len(mapping_dict[(entity_id,doc_id)]["kb10"])
            cnt+=1
            if domain=='attraction':
                average2+=len(mapping_dict[(entity_id,doc_id)]["entdoc"])+len(mapping_dict[(entity_id,doc_id)]["kb9"])+len(mapping_dict[(entity_id,doc_id)]["kb10"])
                cnt2+=1
            #print(domain,entity_id,doc_id)
    if cnt2==0:
        cnt2=1
    print("\nseen entitiy : %d / %d\nseen attraction entitiy: %d\naverage_variation=%.3f\naverage attaction varation=%.3f"%(cnt,kb10_entitiy_length,cnt2,average/cnt,average2/cnt2))
...
def mapping_variation(mapping_dict,bart_gen,t5_gen,min_count):
    ...
    for (domain,entity_id,doc_id) in kb10_every_entity:
        ...
        if entity_id not in mapping_uttr2:
            mapping_uttr2[entity_id]={}
        if doc_id not in mapping_uttr2[entity_id]:
            mapping_uttr2[entity_id][doc_id]=[]
        
        variation_cnt=0
        #첫번째 mapping dict 사용 - dstc9 dialog
        for kb9_ent_doc in mapping_dict[(entity_id,doc_id)]['entdoc']:
            kb9_ent_doc= tuple(map(int,kb9_ent_doc))
            if kb9_ent_doc in entdoc2uttr:
                dstc9_log_list=entdoc2uttr[kb9_ent_doc]
                mapping_uttr2[entity_id][doc_id].extend(dstc9_log_list)
                variation_cnt+=len(dstc9_log_list)
        
        #print("\ndstc9_log=%d"%variation_cnt)
        
        if variation_cnt<min_count:
            if entity_id in bart_gen:
                if doc_id in bart_gen[entity_id]:
                    variation_list=bart_gen[entity_id][doc_id]
                    mapping_uttr2[entity_id][doc_id].extend(variation_list)
                    variation_cnt+=len(variation_list)
            #print("\nbart=%d"%variation_cnt)
        #t5
        if variation_cnt<min_count:
            if entity_id in t5_gen:
                if doc_id in t5_gen[entity_id]:
                    variation_list=t5_gen[entity_id][doc_id]
                    mapping_uttr2[entity_id][doc_id].extend(variation_list)
                    variation_cnt+=len(variation_list)
            #print("\nt5=%d"%variation_cnt)
        ...
        #dstc9 faq 사용
        if variation_cnt<min_count:
            for domain,idx in mapping_dict[(entity_id,doc_id)]['kb9']:
                mapping_uttr2[entity_id][doc_id].append(kb9_domain_q_dict[domain][idx])
                variation_cnt+=1
            #print("\ndstc9_faq=%d"%variation_cnt)
        #dstc10 faq사용        
        if variation_cnt<min_count:
            for domain,idx in mapping_dict[(entity_id,doc_id)]['kb10']:
                mapping_uttr2[entity_id][doc_id].append(kb10_domain_q_dict[domain][idx])
                variation_cnt+=1
            #print("\ndstc10_faq=%d"%variation_cnt)
        #나머지
        if variation_cnt<min_count:
            print("cautious",(entity_id,doc_id))
        ...
        if domain=='attraction':
            ...
            attr_cnt+=1
        #print(cnt)
    print("최종 variation average = %.2f\n"%(total_cnt/len(kb10_every_entity)))
...
#마지막 없다면 kb10
mapping_dict=mapping_kb10_kb10(mapping_dict,num_k)
check_kb10_entity_link(mapping_dict)
#참고로 attraction 230개
...
def make_single_faq_dialog(dialog,faqs,entity_idx,doc_idx_list,selected_uttr_pos,domain,entity_name):
    ...
    for i in doc_idx_list:
        ...
        user_faq_dict['text']=random.sample(mapping_uttr2[entity_idx][i],1)[0].lower()
...
#여기서 이제 넣을 거임.
with open('mydata/new_knowledge_tr.json', 'r') as f:
            ...
            for domain in possible_loc:
                if possible_loc[domain] :
                    while True:
                        ...
                        if entity_idx in mapping_uttr2:
                            break

                    faqs=new_knowledge[domain][entity_idx]['docs']
                    entity_name=new_knowledge[domain][entity_idx]['name']

                    candidates_doc=list(mapping_uttr2[entity_idx].keys())
...
for dialog in tqdm(augmented_domain_session_logs):
    ...
    for user_idx in faq_uttr_idx_list:
        new_logs.append(dialog_form[:user_idx])
        new_labels.append({
        'target': True,
        "knowledge": [
            {
    ...
    #negative sample
    try:
        user_idx_list=random.sample(normal_uttr_idx_list,len(faq_uttr_idx_list))
    except:
        user_idx_list=normal_uttr_idx_list
