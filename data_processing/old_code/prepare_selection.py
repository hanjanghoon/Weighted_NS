...
import torch
import os
from normalize import normalize_numbers 
random.seed(1228)
np.random.seed(1228)
    ...
    for i in range(20):
        ...
        for i in doc_idx_list:
            ...
            if mapping_uttr2['body'][entity_idx][i]:
                ...
                sys_faq_dict['text']=faqs[str(i)]['body'].lower()
...
try:
    if not os.path.exists("data_detection/train"):
            os.makedirs("data_detection/train")
except OSError:
    print ('Error: Creating directory. ' +  "data_detection/train")


with open('data_detection/augmented_domain_session_logs.json', 'w') as f:
    json.dump(augmented_domain_session_logs, f, indent=4)


#대화 세션 쪼개기.
with open('data_detection/augmented_domain_session_logs.json', 'r') as f:
...
for dialog in tqdm(augmented_domain_session_logs):
    ...
    neg_idx=0
    for i in range(1,len(dialog)):
        #text=normalize_numbers(dialog[i]['text'])
...
#new_logs+=val_logs[100:]
#new_labels+=val_labels[100:]


with open('data_selection/train/logs.json', 'w') as f:
    json.dump(new_logs, f, indent=4)
print("# of new_logs %d"%(len(new_logs)))


with open('data_selection/train/labels.json', 'w') as f:
    json.dump(new_labels, f, indent=4)
print("# of new_labels %d"%(len(new_labels)))



with open('data_selection/val/logs.json', 'w') as f:
    json.dump(val_logs, f, indent=4)

with open('data_selection/val/labels.json', 'w') as f:
    json.dump(val_labels, f, indent=4)


def knowledge_normalize(target_knowledge):
    for domain in target_knowledge:
        if domain=='taxi' or domain=='train':
            continue
        for entity_id in target_knowledge[domain]:
            for doc_id in target_knowledge[domain][entity_id]['docs']:
                target_knowledge[domain][entity_id]['docs'][doc_id]['title']=normalize_numbers(target_knowledge[domain][entity_id]['docs'][doc_id]['title'])
                target_knowledge[domain][entity_id]['docs'][doc_id]['body']=normalize_numbers(target_knowledge[domain][entity_id]['docs'][doc_id]['body'])
                
    with open('data_selection/knowledge.json', 'w') as f:
        json.dump(target_knowledge, f, indent=4)

#knowledge_normalize(new_knowledge)
with open('data_selection/knowledge.json', 'w') as f:
