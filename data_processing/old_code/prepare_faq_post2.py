                    ...
                    if topk_val > min_p:
                        ...
                        for key in kb10_key_list:
                            if key not in mapping_dict[type]:
                                mapping_dict[type][key]={"entdoc":[],"kb9":[],"kb10":[],"kb10_entity_doc_idx":[]}
                ...
                for topk_val, topk_idx in zip(topk_values,topk_indices):#여기서 나중에 top k
                    ...
                    if topk_val > min_p:
                        ...
                        for key in kb10_key_list:
                            ...
                            idx_tuple=(domain,int(topk_idx))
                            kb10_entity_doc_idx_list=kb10_mapping_dict[domain][type][int(topk_idx)]
                            if idx_tuple not in mapping_dict[type][key]["kb10"]:
                                mapping_dict[type][key]["kb10"].append(idx_tuple)
                                mapping_dict[type][key]["kb10_entity_doc_idx"].extend(kb10_entity_doc_idx_list)
...
for (domain,entity_id,doc_id) in tqdm(kb10_every_entity):
    ...
    #negative sampling 하면 안되는 정답이 되는것들. 제외.

    if mapping_uttr2['title'][entity_id][doc_id]:
            userlist=random.sample(mapping_uttr2['title'][entity_id][doc_id],min( 11,len(mapping_uttr2['title'][entity_id][doc_id]) ) )
            userlist.append(title)
    else:
        userlist=[title]

    if mapping_uttr2['body'][entity_id][doc_id]:
            systemlist=random.sample(mapping_uttr2['body'][entity_id][doc_id],min( 11,len(mapping_uttr2['body'][entity_id][doc_id]) ))
            systemlist.append(body)
    else:
        systemlist=[body]
    
    #entitiy name을 포함하는 knowledge 구분 
   
    kb10_body_both=list(set(mapping_dict['title'][(entity_id,doc_id)]['kb10_entity_doc_idx']))
    kb10_body_indep=[]
    kb10_body_dep=[]
    for (entity_id2,doc_id2) in kb10_body_both :
        #이슈 이경우에는 앞에 Title 정보를 사용하지 않아야한다.
        if  kb10[domain][entity_id2]['name'].lower() not in kb10[domain][entity_id2]['docs'][doc_id2]['body'].lower():
            kb10_body_indep.append("{}__{}__{}".format(domain,entity_id2,doc_id2))
        kb10_body_dep.append("{}__{}__{}".format(domain,entity_id2,doc_id2))
    
    #query는 title 기준으로..
    for i in range(len(userlist)):
        idx=i%len(userlist) #길이 다를경우 걍 나머지로.
        idx2=i%len(systemlist)
        user=userlist[idx]
        system=systemlist[idx2]
        user,userflag=replace_domain(user,domain,entity_name)
        system,systemflag=replace_domain(system,domain,entity_name)
        same_knowledge=[]
        if userflag:
            kb10_body=kb10_body_indep
            
        else:
            kb10_body=kb10_body_dep
...
with open('faq_posttrain2/train/logs.json', 'w') as f:
    json.dump(new_logs, f, indent=4)
print("# of new_logs %d"%(len(new_logs)))
with open('faq_posttrain2/train/labels.json', 'w') as f:
