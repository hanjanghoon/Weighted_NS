...
for dialog in tqdm(augmented_domain_session_logs):
    ...
    for i in range(1,len(dialog)):
        ...
        dialog_form.append({'speaker':dialog[i]['speaker'],'text':text})
    ...
    for user_idx in faq_uttr_idx_list:
        ...
        "response": normalize_numbers(dialog_form[user_idx]['text'])
        })
...
with open('data_detection/train/logs.json', 'w') as f:
    json.dump(new_logs, f, indent=4)
print("# of new_logs %d"%(len(new_logs)))


with open('data_detection/train/labels.json', 'w') as f:
    json.dump(new_labels, f, indent=4)
print("# of new_labels %d"%(len(new_labels)))



with open('data_detection/val/logs.json', 'w') as f:
    json.dump(val_logs, f, indent=4)

with open('data_detection/val/labels.json', 'w') as f:
